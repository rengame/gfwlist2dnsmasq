...
Valid options are:
    ...
    -t, --out-type
                Convert Gfwlist into dnsmasq rule,smartdns rule,domain list
...
get_args(){
    ...
    if [ $OUT_TYPE = 'smartdns' ]; then
        # Check smartdns group name
        if [ -z $SMARTDNS_GROUP ]; then
            _red 'Error: Please specify the name to the smartdns group(using -g/--group argument).\n'
            exit 1
        fi
        # Check ipset name
        if [ -z $IPSET_NAME ]; then
...
ipset=/\1/'$IPSET_NAME'#g' > $CONF_TMP_FILE
        ...
        echo '# '$OUT_TYPE' rules generated by gfwlist' > $OUT_TMP_FILE
        echo "# Last Updated on $(date "+%Y-%m-%d %H:%M:%S")" >> $OUT_TMP_FILE
        echo '# ' >> $OUT_TMP_FILE
        cat $CONF_TMP_FILE >> $OUT_TMP_FILE
        cp $OUT_TMP_FILE $OUT_FILE
    elif [ $OUT_TYPE = 'smartdns' ]; then
    # Convert domains into smartdns rules
        if [ $WITH_IPSET -eq 1 ]; then
            _green 'Ipset rules included.'
            sort -u $DOMAIN_FILE | $SED_ERES 's#(.+)#nameserver /\1/'$SMARTDNS_GROUP'\
ipset /\1/'$IPSET_NAME'#g' > $CONF_TMP_FILE
        else
            _green 'Ipset rules not included.'
            sort -u $DOMAIN_FILE | $SED_ERES 's#(.+)#nameserver /\1/'$SMARTDNS_GROUP'#g' > $CONF_TMP_FILE
        fi
